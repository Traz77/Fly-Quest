# FlyQuest
A 2D Grid World game where a "Fly" agent learns to find a "Human" target.
Started with implementing the Game Engine and used Supervised Learning (Poly-Regression) to predict distances. (used for checking our pipeline works on a known problem).

## The Reinforcement Learning Journey

### Phase 1: Tabular Q-Learning (The Dictionary Brain)
We built a Reinforcement Learning engine from scratch using an Object-Oriented approach.
...
When we made the Human move randomly, the state space exploded to **10,000 possible states** `(fly_x, fly_y, human_x, human_y)`.
While the Fly could still learn this by training for 1,000,000 episodes, the Tabular approach hit its limit. If we added moving Spiders, the state space would become 10-Billion, requiring more RAM than exists on a computer.

### Phase 3: Deep Q-Learning (PyTorch)
To solve the Curse of Dimensionality, we transitioned to **Deep Q-Networks (DQN)**.
Instead of memorizing every possible combination in a dictionary, we built a Neural Network to *generalize* the rules of the game.
...
- **Experience Replay Buffer**: Instead of learning from 1 step at a time, we store 10,000 past experiences in a `deque`. The AI learns from a random batch of 64 experiences simultaneously to prevent "Catastrophic Forgetting."
...
Even after applying the Target Network and step limit fixes, the Fly still behaved erratically. Upon investigation, we discovered a **fundamental flaw in the state representation**: the state `(fly_x, fly_y, human_x, human_y)` gives the Neural Network zero information about spider positions. The Fly was essentially navigating a **minefield blindfolded** — randomly dying to invisible obstacles that change location every episode. The Neural Network cannot learn to avoid threats it cannot see.

**Solution: Curriculum Learning** — temporarily remove Spiders from training so the Neural Network can first master the core skill of chasing a moving target. Once it proves it can reliably hunt the Human, we reintroduce Spiders with their positions included in the state vector.
